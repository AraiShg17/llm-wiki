...
3. Wiki自体を更新、整理、検査する場合は [operations.md](operations.md)
4. UI、API、DB、テストなど作業種別が明確な場合は `skills/` の該当ファイル
5. Todo、認証、通知など機能が明確な場合は `features/` の該当ファイル
6. システム全体の構造が必要な場合だけ [architecture.md](architecture.md)
7. 設計思想やUI方針が必要な場合だけ [design.md](design.md)
8. 実装ルールが必要な場合だけ [coding_rules.md](coding_rules.md)
...
| [operations.md](operations.md) | Wiki更新、Query保存、Lintの運用 | AI向け資料を保守するとき |
| [state.md](state.md) | 現在の作業状況 | 作業開始時、既存作業の影響を確認したいとき |
| [log.md](log.md) | AIによる更新履歴 | 直近のドキュメント変更を追うとき |
...
- Wiki構成の健康診断だけ: `operations.md` と `log.md` だけ読む
...
- `log.md` は履歴であり、現在状態の判断には `state.md` を優先してください。
- `features/` は機能仕様、`skills/` は作業方法です。混同しないでください。
- 調査で得た知見が次回も使える場合だけ、[operations.md](operations.md) に従ってWikiへ書き戻してください。
